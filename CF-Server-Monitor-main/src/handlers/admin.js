...
async function deleteServer(db, id) {
  // 删除服务器时一并清理其历史指标，避免留下孤儿数据
  // （metrics_history 与 servers 之间未定义外键，原逻辑只删 servers 会遗留孤儿行）
  try {
    try {
    ...
    if (data.action === 'get_settings') {
      ...
      const serversWithStatus = servers.map(server => {
        ...
        if (isOnline) {
          ...
          stats.total_ram += parseFloat(item.ram_used) || 0;
          stats.total_disk += parseFloat(item.disk_used) || 0;
      ...
      await env.DB.prepare(`
        INSERT INTO servers 
        (id, name, server_group, sort_order, ssh_host, ssh_port, ssh_user, ssh_password, agent_id) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).bind(id, name, group, sortOrder, '', 22, '', '', '').run();
      ...
      const { id, name, server_group, price, expire_date, bandwidth, traffic_limit, traffic_calc_type, reset_day, collect_interval, report_interval, ping_mode, is_hidden, ssh_host, ssh_port, ssh_user, ssh_password, agent_id } = data;
      if (!id || !isValidUUID(id)) {
        return createBadRequestResponse('invalidServerId');
      }
      ...
      try {
        await env.DB.prepare(`
          UPDATE servers
          SET name = ?, server_group = ?, price = ?, expire_date = ?, bandwidth = ?, traffic_limit = ?, traffic_calc_type = ?, reset_day = ?, collect_interval = ?, report_interval = ?, ping_mode = ?, is_hidden = ?, ssh_host = ?, ssh_port = ?, ssh_user = ?, ssh_password = ?, agent_id = ?
          WHERE id = ?
        `).bind(
          name || '',
          ...
          is_hidden || '0',
          ssh_host || '',
          ssh_port !== undefined && ssh_port !== null && ssh_port !== '' ? ssh_port : 22,
          ssh_user || '',
          ssh_password || '',
          agent_id || '',
          id
